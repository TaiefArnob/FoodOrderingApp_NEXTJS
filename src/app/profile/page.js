...
export default function ProfilePage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (update) {
        await update({ user: updateData.user });
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 via-white to-amber-100 px-4 pb-10">
      ...
      <div className="max-w-md w-full bg-white rounded-3xl p-8 shadow-xl border border-amber-200">
          ...
          <div className="relative w-[140px] h-[140px]">
            {preview ? (
              <Image
                ...
                width={140}
                height={140}
                className="rounded-full object-cover border-4 border-amber-400 shadow-lg"
                alt="Profile Preview"
              />
            ) : user?.profileImage ? (
              <Image
              ...
              <FaUserCircle className="w-full h-full text-gray-400 rounded-full border-4 border-amber-400 shadow-lg" />
            )}
        ...
        <div className="mt-8 space-y-4">
          <div>
            <label className="block text-[#4b0000] font-semibold mb-1">Name</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-[#4b0000] font-semibold mb-1">Phone</label>
            <input
              type="tel"
              pattern="^(\+8801[3-9]\d{8})$" // Bangladeshi phone format
          ...
          <div>
            <label className="block text-[#4b0000] font-semibold mb-1">Address</label>
            <textarea
              rows={3}
              className="w-full px-4 py-2 border border-amber-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-400 text-[#4b0000]"
              value={address}
